...
copy_file_to_s3() {
  ...
  mc cp "$file_source" "$S3_CLIENT_PREFIX/$S3_BUCKET_NAME/$(compose_folder_name)/$(basename "$file_destination_name")"
}

copy_folder_to_s3() {
  folder_source=${1}
  echo "copying $folder_source -> $S3_BUCKET_NAME/$(compose_folder_name)"
  mc cp -r "$folder_source" "$S3_CLIENT_PREFIX/$S3_BUCKET_NAME/$(compose_folder_name)"
}
